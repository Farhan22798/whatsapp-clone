...
const GroupsScreen = ({ navigation }) => {
    ...
    const fetchGroups = async (keyword = '', isInitial = false) => {
        if (isInitial) setInitialLoading(true);
        setLoading(true);
        let builder = new CometChat.GroupsRequestBuilder().setLimit(30).joinedOnly(true);
        if (keyword) builder = builder.setSearchKeyword(keyword);
        const groupRequest = builder.build();

        try {
            const groupList = await groupRequest.fetchNext();
            setGroups(groupList);
        } catch (err) {
    ...
    const renderItem = ({ item }) => (
        ...
        >
            <View style={styles.userContent}>
                <Image
                    source={{
                        uri: item.getIcon() || 'https://icons.veryicon.com/png/o/miscellaneous/admin-dashboard-flat-multicolor/user-groups.png',
                    }}
                    style={styles.userAvatar}
                />
                <View style={styles.userInfo}>
                    <Text style={styles.userName}>{item.getName()}</Text>
                    <Text style={styles.userSubtitle}>{item.getGuid()}</Text>
                </View>
            </View>
        </TouchableOpacity>
    );
